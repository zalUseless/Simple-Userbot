...
@register(outgoing=True, pattern="^.sending (.*)")
    ...
    async for e in bot.iter_dialogs():
        if e.is_group:
            ...
            try:
                ...
                d += 1
    await c.edit(f"**Successfully Sending Message To** `{done}` **Group, Failed To Send Message To** `{er}` **Group**")

CMD_HELP.update({"broadcast": "`.sending <pesan>`\\nPenjelasan: Global Broadcast mengirim pesan ke Seluruh Grup yang Lord Masuki."})
